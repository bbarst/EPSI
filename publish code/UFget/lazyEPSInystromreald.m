...
max_iter = 50;
% [U1,E1] = RandSubspace(U0,A,max_iter,q,Sigma,1/8,1/4);
[U2,E2] = LazyEPSI(U0,A,max_iter,q,Sigma,1/8,1/4);
...
lgd = legend(name,'FontSize',16);
lgd.LineWidth = 2;
title('Subspace Iteration','FontSize',36)
%xlabel('Iteration','FontSize',36)
%ylabel('Error','FontSize',36)
ylim([1e-8 1]);
...
saveas(gcf,'lazyepsi.png')




function [u1,E] = RandSubspace(u0,A,max_iter,q,A_eig,c1,c2)
    [m, n] = size(A);
    E = zeros(q,max_iter);
    Omega = randn(n, q + 1);   
    Y = A * Omega;            
...
function [U, Lambda_hat] = randomizedNystrom(A, ell)
...
%   Lambda_hat  : ell-by-ell diagonal matrix containing approximate eigenvalues
    ...
    [Omega, ~] = qr(Omega, 0);         % Thin QR gives n-by-ell Q with orthonormal columns
    
    % 2) Form Y = A * Omega
    Y = A'*(A * Omega);                     % n-by-ell
    for i = 1:k
        Y = A'*(A * Y);
    end
    ...
    sigma2 = (diag(Sigma).^2 - nu).^(1/(k+1));   % 1-by-ell
    sigma2_shifted = max(sigma2, 0); % clamp below at 0
    Lambda_hat = diag(sigma2_shifted);
...
function [u1,E] = LazyEPSI(u0,A,max_iter,q,A_eig,c1,c2)
n = size(A,2);
E = zeros(q,max_iter);

[Unys,Lambda_hat]= randomizedNystrom(A, 30);
UnysT = Unys';
...
for i=1:max_iter
    ...
    for j=1:q
        ...
        F_g = WTW-diag((1./diag(Lambda_hat))/ll);
